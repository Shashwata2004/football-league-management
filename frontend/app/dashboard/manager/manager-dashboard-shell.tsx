...
export default function ManagerDashboardPage() {
  ...
  useEffect(() => {
    Object.values(managerSectionPaths).forEach((path) => router.prefetch(path));
  }, [router]);
  ...
  async function load(requestedTeamId?: string) {
    const requestId = ++loadRequestIdRef.current;
    setLoading(true);
    setMessage("");
    try {
      const dashboardPath = requestedTeamId
        ? `/manager/dashboard?teamId=${encodeURIComponent(requestedTeamId)}`
        : "/manager/dashboard";
      const [dashboard, leagueData] = await Promise.all([
        api<DashboardPayload>(dashboardPath),
        api<{ leagues: League[] }>("/manager/leagues"),
      ]);
      if (requestId !== loadRequestIdRef.current) return;
      const activeId = dashboard.active_team?.id ?? "";
      let nextTeamDetail: typeof teamDetail = null;
      if (activeId) {
  ...
  async function changeTeam(teamId: string) {
    ...
    setHasPresentedAvailabilityNotice(false);
    if (routeMatchId) router.push("/dashboard/manager/results");
    await load(teamId || undefined);
  }
  ...
  async function submitRejectedLineupAgain() {
    await closeAvailabilityNotice();
    navigateSection("Submit Lineup");
  }
  ...
  useEffect(() => {
    setStatsSeasonId(activeSeason?.id ?? "");
  }, [activeSeason?.id]);
  const statsSeason =
  ...
  useEffect(() => {
    ...
    const refreshUnreadLineupRejections = async () => {
      try {
        const { messages } = await api<{ messages: MessageRecord[] }>(
          `/manager/messages/unread-lineup-rejections?teamId=${encodeURIComponent(activeTeam.id)}`,
        );
        if (!alive || messages.length === 0) return;
        setPayload((current) => {
          if (!current) return current;
          const incomingIds = new Set(messages.map((item) => item.id));
          const merged = [
            ...messages,
            ...current.messages.filter((item) => !incomingIds.has(item.id)),
          ]
            .sort(
              (a, b) =>
        ...
        // A transient notification check must not interrupt manager workflows.
      }
    };

    const intervalId = window.setInterval(
      () => void refreshUnreadLineupRejections(),
      15_000,
    );
    const refreshOnFocus = () => void refreshUnreadLineupRejections();
  ...
  useEffect(() => {
    ...
    const unreadNotices = payload.messages.filter(
      (item) =>
        !item.read_at &&
        !dismissedAvailabilityNoticeIds.has(item.id),
    );
    const lineupRejection = unreadNotices.find(
      (item) => item.related_type === "LINEUP_BLOCK",
    );
    const availabilityUpdate = hasPresentedAvailabilityNotice
      ? undefined
      : unreadNotices.find((item) =>
          /injur|suspend|out for|submit your lineup|today is your match|matchday/i.test(
            `${item.related_type} ${item.message}`,
          ),
        );
    const notice = lineupRejection ?? availabilityUpdate;
    if (notice) {
      setAvailabilityNotice(notice);
      if (notice.related_type !== "LINEUP_BLOCK") {
    ...
    >
      ...
      <main className="min-h-screen lg:pl-72">
        ...
        <div className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-8 lg:px-8">
          ...
          {loading ? (
            ...
            <>
              <div hidden={Boolean(selectedFixture) || Boolean(routeMatchId)}>
                {renderedSections.map((renderedSection) => (
                  ...
                  >
                    <SectionView
                      ...
                      summary={
                        teamDetail?.squad_summary ??
                        payload?.squad_summary ??
                        null
                      }
                      ...
                      onRefresh={() =>
                        void load(activeTeam?.id ?? undefined).catch((error) =>
      ...
      {generateOpen && activeTeam && (
        <GenerateSquadModal
          ...
          onGenerated={() => {
            setGenerateOpen(false);
            void load(activeTeam.id).catch((error) =>
              setMessage(error.message),
            );
          }}
        />
      ...
      {availabilityNotice ? (
        <AvailabilityNoticeModal
          ...
          {...(availabilityNotice.related_type === "LINEUP_BLOCK"
            ? { onSubmitAgain: () => void submitRejectedLineupAgain() }
            : {})}
        />
      ) : null}
    </div>
...
function MessagesSection({
  ...
  const reloadMessages = useCallback(async () => {
    ...
    const { messages: full } = await api<{ messages: MessageRecord[] }>(
      `/manager/messages?teamId=${encodeURIComponent(activeTeam.id)}`,
    );
    setMessages(full);
    return full;
  }, [activeTeam?.id]);
  ...
  }, [activeTeam?.id, dashboardMessages]);
...
function AvailabilityNoticeModal({
  ...
  const isLineupRejection = message.related_type === "LINEUP_BLOCK";
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="manager-notice-title"
      className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/50 p-4 backdrop-blur"
    >
      <div className="w-full max-w-md rounded-2xl bg-white p-4 shadow-2xl sm:rounded-[2rem] sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            {isLineupRejection ? (
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-red-50 text-red-600 ring-1 ring-red-100">
        ...
        {isLineupRejection ? (
          <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button
              type="button"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--team-primary)] px-5 py-3 text-sm font-black text-[var(--team-primary-text)] shadow transition hover:-translate-y-0.5"
