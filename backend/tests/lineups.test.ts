...
function lineup(
  overrides: Partial<LineupSubmissionInput> = {},
): LineupSubmissionInput {
  return {
...
describe("lineup validation", () => {
  it("accepts exactly 11 starters with one goalkeeper", () => {
    expect(
      validateLineupSubmission(
        lineup(),
        registered,
        "4c74634e-9cc7-4a63-a8ec-5e3db8a92f11",
      ),
    ).toBe(true);
  });
  ...
  it("rejects duplicate player", () => {
    const input = lineup();
    input.players[1]!.player_registration_id =
      input.players[0]!.player_registration_id;
    expect(() =>
      validateLineupSubmission(
        input,
        registered,
