...
import { FootballPosition, PlayerPosition, type PlayerAbilityRating } from "@flms/shared";
...
function coarseToFootballPosition(position?: PlayerPosition | string | null): FootballPosition {
  if (position === PlayerPosition.GK || position === "GK") return FootballPosition.GK;
  if (position === PlayerPosition.DEF || position === "DEF") return FootballPosition.CB;
  if (position === PlayerPosition.MID || position === "MID") return FootballPosition.CM;
  return FootballPosition.ST;
}

function abilityUpdate(registration: RegistrationRow, tier: PlayerAbilityRating) {
  const footballPosition = registration.football_position ?? coarseToFootballPosition(registration.position);
  const generated = generateAbilityScores(tier, footballPosition, `${registration.id}:${tier}:${footballPosition}:rebalance-v2`);
  const base = {
...
const registrationIds = [...new Set(abilityRows.map((row) => row.player_registration_id))];
...
const { data: registrations, error: registrationError } = await supabase
  ...
  .select("id,player_id,season_id,team_registration_id,position,football_position")
  .in("id", registrationIds);

if (registrationError) throw registrationError;

const registrationById = new Map(((registrations ?? []) as RegistrationRow[]).map((row) => [row.id, row]));
let updated = 0;
...
console.log(`Rebalanced ${updated} player ability rows with position-specific ranges.`);
